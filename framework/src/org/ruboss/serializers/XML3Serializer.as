package org.ruboss.serializers {
  ...
  public class XML3Serializer implements ISerializer {

    protected var state:ModelsStateMetadata;

    public function XML3Serializer() {
     ...
     */
    public function unmarshall2(object:Object):Object {
      ...
      try {
        ...
        if (xmlFragment.@type == "array") {
          ...
          if (state.fqns[objectName]) {
            results.itemType = state.fqns[objectName];
            for each (var node:XML in xmlFragment.children()) {
              results.push(unmarshallXMLNode(node));
            }
          }
          return results;
        } else {
    ...
    private function unmarshallXMLNode(node:XML, type:String = null):Object {
      ...
      var fqn:String = (!type) ? state.keys[localName] : type;
        ...
        if (targetName.search(/.*_id$/) != -1) {
          ...
          if (checkName == "parent") {
            ...
            if (!RubossUtils.isEmpty(polymorphicRef)) {
              ...
              if (state.types[polymorphicRefName]) {
                targetType = state.types[polymorphicRefName];
                isRef = true;
              } else {
                throw new Error("Polymorphic type: " + polymorphicRef + " is not a valid Ruboss Model type.");
              }
            } else if (state.refs[fqn][targetName]) {
              targetType = state.refs[fqn][targetName];
              isRef = true;
            }
          }
        } else if (element.@type == "array" && state.refs[fqn][targetName]) {
          isNestedArray = true;        
        }
