...
task :doc do
  ...
  system("#{get_executable('asdoc')} +configname=air -main-title 'RestfulX Framework 1.2.2 API Documenation' -source-path framework/src -doc-sources framework/src -library-path+=#{libs.join(',')} #{packages.join(" ")} -output doc/api")
end
